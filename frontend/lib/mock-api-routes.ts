...
const filteredHomework = (query: URLSearchParams) => {
  ...
  if (deadlineFrom)
    items = items.filter((item) => item.deadline >= deadlineFrom);
  if (deadlineTo) items = items.filter((item) => item.deadline <= deadlineTo);

  return items;
};

export const resolveMockRequest = <T>(
  endpoint: string,
  ...
  if (method === "GET") {
    if (path === "/subjects" || path === "/api/v1/subjects")
      return [...state.subjects] as T;
    if (path === "/api/v1/schedule" || path === "/schedule")
      return [...state.schedule] as T;
    ...
    if (path === "/api/v1/analytics/warnings")
      return { warnings: MOCK_WARNINGS } as T;
    if (path === "/api/v1/reminders/pending")
      return MOCK_PENDING_REMINDERS as T;
  }

  if (
    (path === "/api/v1/subjects" || path === "/subjects") &&
    method === "POST"
  ) {
  ...
  if (
    (path.startsWith("/api/v1/subjects/") || path.startsWith("/subjects/")) &&
    method === "PUT"
  ) {
    const id = path.replace("/api/v1/subjects/", "").replace("/subjects/", "");
    state.subjects = state.subjects.map((s) =>
      s.id === id ? { ...s, ...(body as SubjectUpdate) } : s,
    );
  ...
  if (
    (path === "/api/v1/schedule" || path === "/schedule") &&
    method === "POST"
  ) {
  ...
  if (
    (path.startsWith("/api/v1/schedule/") || path.startsWith("/schedule/")) &&
    method === "PUT"
  ) {
    const id = path.replace("/api/v1/schedule/", "").replace("/schedule/", "");
    state.schedule = state.schedule.map((slot) =>
      slot.id === id ? { ...slot, ...(body as ScheduleSlotUpdate) } : slot,
    );
  ...
  if (path.endsWith("/generate-steps") && method === "POST") {
    const id = path
      .replace("/api/v1/homework/", "")
      .replace("/generate-steps", "");
    const target = state.homework.find((item) => item.id === id);
    const steps = [
      {
        id: uid(),
        title: "Прочитать условие и выделить главное",
        order: 1,
        is_completed: false,
      },
      {
        id: uid(),
        title: "Составить короткий план решения",
        order: 2,
        is_completed: false,
      },
      {
        id: uid(),
        title: "Проверить и оформить ответ",
        order: 3,
        is_completed: false,
      },
    ];
  ...
  if (path.startsWith("/api/v1/homework/") && method === "PUT") {
    ...
    state.homework = state.homework.map((item) =>
      item.id === id ? { ...item, ...(body as object) } : item,
    );
  ...
  if (path.startsWith("/api/v1/homework/") && method === "DELETE") {
    ...
    state.reminders = state.reminders.filter(
      (reminder) => reminder.homework_id !== id,
    );
    return undefined as T;
  }
  ...
  if (path === "/api/v1/reminders" && method === "POST") {
    const payload = body as ReminderCreate;
    const targetHomework = state.homework.find(
      (item) => item.id === payload.homework_id,
    );
    ...
    const item = {
      ...
      homework: targetHomework
        ? { id: targetHomework.id, title: targetHomework.title, subject: null }
        : null,
  ...
  if (path.startsWith("/api/v1/reminders/") && method === "PUT") {
    ...
    state.reminders = state.reminders.map((reminder) =>
      reminder.id === id
        ? { ...reminder, ...(body as ReminderUpdate) }
        : reminder,
    );
  ...
  if (path.startsWith("/api/v1/reminders/") && method === "DELETE") {
    state.reminders = state.reminders.filter(
      (reminder) => reminder.id !== path.replace("/api/v1/reminders/", ""),
    );
    return undefined as T;
  }
